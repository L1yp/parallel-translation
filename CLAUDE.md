...
- **`vocab.js`** —— Service Worker 内的 IndexedDB 生词本数据层。库名 `itl-vocab`，store `entries`（keyPath `id`），索引 `byNormalized = [normalized, targetLang]`（unique）与 `byCreatedAt`。导出 `vocabAdd / vocabRemove / vocabRemoveByWord / vocabCheck / vocabList / vocabUpdateNote / vocabClearAll / vocabExportAll / vocabImport / vocabStats / normalizeWord`。所有错误吞咽返回安全默认值，**永远不阻断翻译主流程**。容量上限 5000 条，超出按 createdAt 升序淘汰最旧。**与 cache 完全独立的库**：cache 是短期 TTL、value 可丢失；生词本是用户长期资产，没有 TTL，只在用户主动清空 / 超容时移除。background.js 暴露 `vocab-add` / `vocab-remove` / `vocab-remove-by-word` / `vocab-check` / `vocab-list` / `vocab-update-note` / `vocab-clear-all` / `vocab-export` / `vocab-import` / `vocab-stats` 十条消息。详细设计见 [docs/vocabulary.md](docs/vocabulary.md)。
...
- **选区上下文（`context`）**：`extractSelectionContext(text)` 拿选区所在块的 textContent，前后各取 ~80 字符，跨节点 / Shadow Root 失败时回退空字符串。**上下文提取要在 selection 还在的时候做**（button 模式下用户点按钮后 selection 可能丢），所以放在 `fetchSelectionTranslation` 的同步路径里，而不是 promise then 里。
